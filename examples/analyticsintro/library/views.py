...
def service_getBooks(request):
    ...
    try:
        ...
        for i, book in enumerate(Book.objects.all()):
            dct[str(i)] = {'isbn':book.isbn, 'author':  book.author, 'year': book.year, 'title':book.title}
...
def service_getInv(request):
    '''service:getInv takes in an HTTP request with a single parameter:
        ...
        returns the inventory records associated with all books

        You can test it out with:
        $ curl -i -X GET http://127.0.0.1:8000/tasks/getInv
    ...
    try:
        ...
        for book in books:
            for inv in Inventory.objects.filter(book = book):
                dct[inv.id] = {'id': book.isbn, 'borrowed': inv.borrowed, 'title':book.title}
